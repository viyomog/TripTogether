    ...
    <>
      <nav className="absolute top-0 left-0 w-full z-50 py-4 md:py-6 font-sans">
        <div className="max-w-[95%] mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          {/* Logo */}
          <Link to="/" onClick={closeMenu} className="text-2xl font-black text-[#f43f5e] drop-shadow-md tracking-tight z-50 relative">
            TripTogether
          </Link>
          ...
          >
            ...
            <span className={`block w-6 h-0.5 bg-white transition-all duration-300 mt-1.5 ${isOpen ? 'opacity-0' : ''}`}></span>
            <span className={`block w-6 h-0.5 bg-white transition-all duration-300 mt-1.5 ${isOpen ? '-rotate-45 -translate-y-3' : ''}`}></span>
          </button>
        </div>
      </nav>
